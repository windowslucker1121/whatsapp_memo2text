...
async def call_whisper(filename: str, data: bytes, mimetype: str):
    ...
    async with ClientSession() as session:
        ...
        async with session.post(
            url=WHISPER_URL, headers=headers, data=form_data
        ) as response:
            logging.debug(
                f"Whisper response: {response.status}, {await response.text()}"
            )
...
async def call_chatgpt(system_prompt: str, prompt: str):
    ...
    async with ClientSession() as session:
        ...
        async with session.post(
            url=CHATGPT_URL, headers=headers, json=json
        ) as response:
            logging.debug(
                f"ChatGPT response: {response.status}, {await response.text()}"
            )
            response.raise_for_status()
            return await response.json()
